...
fourier = 0;
for i = 1:num_pre
    fourier = fourier + fftshift( abs(fft( [channel_chirp_treshold(i*N-N+1:N*i).*downch, zeros(1,N*(OS-1))] )) );
end
[~, ind1] = max( fourier );
...
[channel_chirp_realign] = LORA.DPHI_COMP(channel_chirp_treshold, dphi1);
...
summ = 0;
for i = 1:num_pre
    fourier = fft((channel_chirp_realign(i*N-N+1:N*i)).*downch);
    [~, ind1] = max( abs(fourier) );
    ind1_win = (-1:1)+ind1;
    [ind1_win] = LORA.CYC_SHIFT(ind1_win);
    summ = summ+-real( (fourier(ind1_win(3))-fourier(ind1_win(1)))/(2*fourier(ind1_win(2))-fourier(ind1_win(3))-fourier(ind1_win(1))) );
end
STOfrac = summ/num_pre;
