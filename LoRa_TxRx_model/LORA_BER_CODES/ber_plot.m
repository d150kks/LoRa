...
%% ================================= Переменные
% lora_crc_dd_ber = load("lora_crc_dd_ber.mat").BER;
% lora_crc_ber = load("lora_crc_ber.mat").BER;
% snr_crc = load("snr_crc.mat").snr;
% 
% lora_crc_ldpc_ber = load("lora_crc_ldpc_ber.mat").BER;
% lora_crc_polar_ber = load("lora_crc_polar_ber.mat").BER;
% snr = load("snr.mat").snr;
% 
% 
% figure(1)
% semilogy(snr,lora_crc_ldpc_ber,'-*','color','k');
% hold on
% semilogy(snr,lora_crc_polar_ber,'-s','color','r');
% legend('ldpc', 'polar')
% grid
% xlabel('SNR (dB)')
...
% 
% figure(2)
% semilogy(snr_crc,lora_crc_ber,'-*','color','k');
% hold on
% semilogy(snr_crc,lora_crc_dd_ber,'-s','color','r');
% legend('crc', 'crc double')
% grid
% xlabel('SNR (dB)')
% ylabel('Bit Error Rate')
% hold off
...
% lora_grey_ber = load("lora_grey_ber.mat").BER;
% lora_cyc_grey_ber = load("lora_cyc_grey_ber.mat").BER;
% snr = load("snr.mat").snr;
% 
% 
% 
% figure(1)
% % semilogy(snr,lora_grey_ber,'-*','color','k');
% semilogy(snr,lora_cyc_grey_ber,'-*','color','k');
% hold on
...
for nLen = 1:3
    for nRate = 1:4
        semilogy(snr,lora_config_ldpc_ber(:,nLen,nRate),strcat('-',mlist(counter)),'color',clist(counter), 'LineWidth',2);
