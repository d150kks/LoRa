...
for n = 1:length(snr)
    ...
    for iter = 1:nIter
        ...
        [rx_preamble, nvar] = awgn(tx_preamble, snr(n), 'measured');
...
%         nvar=1;
        ...
        for i = 1:num_sym
            ...
            for nBit=1:SF
                ...
                m1 = M1(:, nBit);
                m1(m1==0)=[];
                LLR = -(1/nvar)*(min( (peakMakcor-fourier(grayCode(m0)+1)).^2 ) - min( (peakMakcor-fourier(grayCode(m1)+1)).^2 ));
%                 LLR = -(1/nvar)*(min( (peakMakcor-fourier_soft(m0)).^2 ) - min( (peakMakcor-fourier_soft(m1)).^2 ));
%                 LLR = -(1/nvar)*(min( (sv_cor(i)-grayCode(m0)).^2 ) - min( (sv_cor(i)-grayCode(m1)).^2 ));
