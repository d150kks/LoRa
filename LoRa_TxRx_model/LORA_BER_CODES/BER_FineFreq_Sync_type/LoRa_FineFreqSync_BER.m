...
%         fprintf( 'Err:     %.2f\n', freq_shift-(freq_data{2}+freq_data{3}+freq_data{4}+freq_data{5}))
...
%             fourier_data = fourier_data + (fft( corrected_signal(i*N-N+1:i*N).*downch ));
            supsig = supsig+corrected_signal(i*N-N+1:i*N);
        end
        fourier_data = 0 + abs(fft( supsig.*downch ));

       figure(1)
       stem(abs(fourier_data))
...
% save('STOCFOFine_old_ber.mat','BER')
% save('STOCFOFine_ber.mat','BER')
% save('STOCFOFine_old_Hest_ber.mat','BER')
% save('STOCFOFine_Hest_ber.mat','BER')

% save('STOCFOFine_Hest_NLMS_ber.mat','BER')
